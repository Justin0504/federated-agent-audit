...
specialist---no engineered leaks. The agent included the subject's sensitive
identifiers (SSN, MRN, account/card numbers, diagnoses) in the cross-boundary
hand-off in \textbf{9/12 (75\%)} of workflows, and every hand-off to the
different-purpose external party was a purpose violation (12/12, structural). The
center-blind auditor flagged all of them with \textbf{zero raw content reaching
the center}. This is the deployment-measurement counterpart to AgentLeak on the
multi-tenant setting: left to their own judgment, current agents routinely
over-share a subject's identifiers across organizational boundaries, and the
auditor catches it without seeing the data.
