...
labels and we score our auditor against them. On $\sim$18 held-out scenarios,
\textbf{precision stays 1.0 (no false positives) but recall drops to 0.29
(lexical) / 0.43 (LLM tagger)}---far below the self-authored 1.0. The misses are
dominated by (i) taxonomy scope (the author generates preference/location/
academic leaks; our detector targets health/finance/legal, a configurable set)
and (ii) the author stating inferences explicitly rather than compositionally.
Precision holding at 1.0 shows the detectors do not over-fire; recall is bounded
by category coverage, not the architecture. This is the honest ceiling the
self-authored benchmark hides.
